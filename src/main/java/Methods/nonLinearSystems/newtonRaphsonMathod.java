...
public class newtonRaphsonMathod {
    public static double[][] substituteJacobie(Expression[][] jacobi, double[] values) {
        ...
        for (int i = 0; i < jacobi.length; i++)
            variableValues.put("x" + (i + 1), values[i]);
    ...
    public static double[] newton(Expression[] F, double[] guess, Expression[][] jacobi, int iteration, double error)
            throws Exception {
        if (iteration == 0)
            return guess;
        ...
        for (int i = 0; i < h.length; i++)
            newGuess[i] = guess[i] + h[i];
        if (norm(newGuess, guess) <= error)
            return newGuess;
        else if (iteration == 1)
            throw new Exception("Newton-Raphson method diverges");
        else
            return newton(F, newGuess, jacobi, iteration - 1, error);
    }

    public static void main(String[] args) throws Exception {
        Expression f1 = new ExpressionBuilder("-x1^3+x2").variables("x1", "x2").build(); // params -> -
                                                                                         // Math.pow(params[0], 3) +
                                                                                         // params[1];
        Expression f2 = new ExpressionBuilder("x1^2 + x2^2 - 1").variables("x1", "x2").build();// params ->
                                                                                               // Math.pow(params[0], 2)
                                                                                               // + Math.pow(params[1],
                                                                                               // 2) - 1;
        ...
        Expression[] f = { f1, f2 };
        Expression[][] jacobi = {
                { new ExpressionBuilder("-3 * x1^2").variables("x1", "x2").build(),
                        new ExpressionBuilder("1").variables("x1", "x2").build() },
                { new ExpressionBuilder("2*x1").variables("x1", "x2").build(),
                        new ExpressionBuilder("2 * x2").variables("x1", "x2").build() }
        };
